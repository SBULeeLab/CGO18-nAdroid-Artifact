...
/**
 * Algorithm for computing all paths between a pair of nodes
 * in a directed graph.
 *
...
public class AllPathsBuilder<Node> {
	...
	public AllPathsBuilder(IGraph<Node> graph, Node origNode,
		...
		// 'worklist' is needed only to compute 'relevant' and is
	...
	private List<StringBuffer> getAllNonEmptyPathsTo(Node currNode) {
		...
		if (width > maxTraceWidth) {
			return singletonList("<truncated width=\"" + width +
				"\" limit=\"" + maxTraceWidth + "\"/>");
		}
		int i = 0;
