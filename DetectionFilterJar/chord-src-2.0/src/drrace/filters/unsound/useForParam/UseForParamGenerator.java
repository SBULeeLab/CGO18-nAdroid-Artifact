...
public class UseForParamGenerator {
	...
	// registerIfCmpSet is a Set of registers that are used as parameters for a invoke statement
	private Set<Register> registerParamSet = new HashSet<Register>();
	...
	private boolean rdForParamSetDone = false;
	// Set of Quads (getField and getStatic) that are only using for comparing
	private Set<Quad> rdForParamSet = new HashSet<Quad>();
	
	
	UseForParamGenerator(jq_Method m){
		mMethod = m;
		cfg = m.getCFG();
	...
	public Set<Quad> getRdForParamSet(){
		if(!rdForParamSetDone){
			genRdForParamSet();
			rdForParamSetDone = true;
		}
		return rdForParamSet;
	}
	...
	private void checkAndGenMap(Quad q){
		...
		if(mOperator instanceof Invoke){
			if(mOperator instanceof InvokeStatic) {
				...
				for(int i = 0; i < paramLen; i++){
					...
					if(mRegister.getType().isReferenceType())
						registerParamSet.add(mRegister);
				}
			} else {
				ParamListOperand mParamList = Invoke.getParamList(q);
				int paramLen = mParamList.length();
				assert(paramLen > 0);
				addToRegisterHarmfulUseMap(mParamList.get(0).getRegister(), q);
	...
	private void genRdForParamSet(){
		for(Quad getFieldE : getFieldEs){
			...
			if(trackRegister(mRegister, getFieldE)){
				...
				System.out.println("rdForParam: " + getFieldE);
				rdForParamSet.add(getFieldE);
			}
		}
	}
	...
	private boolean trackRegister(Register mRegister, Quad mQuad){
		...
		if(registerParamSet.contains(mRegister)){
			if(!registerHarmfulUseMap.containsKey(mRegister)){
				return true;
			}else{
